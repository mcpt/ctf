    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            if (self.request.user.has_solved(self.get_object())) or (
                self.request.in_contest and self.request.participation.has_solved(self.get_contest_object())
            ):
                context["status"] = "solved"
            elif (self.request.user.has_attempted(self.get_object())) or (
                self.request.in_contest and self.request.participation.has_attempted(self.get_contest_object())
            ):
