...
if challenge_cluster['connection']['host'] is not None:
    ...
    cluster_config.api_key = {'authorization': 'Bearer {}'.format(challenge_cluster['connection']['token'])}
...
def create_challenge_instance(challenge_spec, problem_id, problem_flag, instance_owner, wait=False):
            ...
            'template': {
                ...
                'spec': {
                    'containers': [{
                        ...
                        | ({'securityContext': {k: v for k, v in challenge_cluster['securityContexts'][container['securityContext']].items() if k != 'apparmorProfile'}} if 'securityContext' in container else {})
                    for container in challenge_spec['containers']],
                    'imagePullSecrets': [{'name': challenge_cluster['imagePullSecrets'][container['image']['imagePullSecret']]} for container in challenge_spec['containers'] if 'imagePullSecret' in container['image']],
                    'autoMountServiceAccountToken': False,
                    'restartPolicy': 'OnFailure',
                    'runtimeClassName': challenge_cluster['runtimeClassNames'][challenge_spec["runtimeClassName"] if 'runtimeClassName' in challenge_spec else 'default'],
                },
            },
        },
    }
    ...
    for port in ports_expose:
        if port['protocol'] == 'HTTP':
            ingress_ports.append(port | {'host': challenge_cluster["domain"].format(generate_identifier())})
        else:
    ...
    if wait:
        ...
        while True:
            time.sleep(1)
            pod_list = pod_v1.get(namespace=challenge_cluster['namespace'], label_selector=f'ctf-instance={instance_id}').items
...
def fetch_challenge_instance(challenge_spec, problem_id, instance_owner):
    ...
    job_list = job_v1.get(namespace=challenge_cluster['namespace'], label_selector=f'ctf-problem={problem_id},ctf-instance-owner={instance_owner}')
    ...
    try:
        nodeport_svc = service_v1.get(namespace=challenge_cluster['namespace'], name=f'{job.metadata.name}-tcp')
    ...
    try:
        clusterip_svc = service_v1.get(namespace=challenge_cluster['namespace'], name=f'{job.metadata.name}-http')
...
def delete_challenge_instance(challenge_spec, problem_id, instance_owner):
    ...
    job_v1.delete(namespace=challenge_cluster['namespace'], name=challenge_instance_status['instance']['name'], propagation_policy='Background')
